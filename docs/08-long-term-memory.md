...
Memory Serviceは`evidence.db`とSemantic Wiki repositoryを単独でwrite所有する。Go CoreとはMemory Outbox/InboxとUnix domain socketでmessageを交換し、DB fileを共有しない。Episode Compilation Jobのlease expiry時はFramework sessionを再開せず、固定input snapshotから新しいRunで最初から再調査する。
...
Memory PlaneはJob開始前に、CoreとGovernanceのOutboxから受信した終端TaskのContract、Progress、Task/Agent Run Event、Child Outcome、Decision、Review、Egress Challenge、Policy Grant、Outbound Transaction、Artifact索引をwatermarkとdigest付きterminal snapshotとしてEvidence DBへ取り込む。そのsnapshotからJob scopeのTEMP/read-only viewを構築してからAgent用connectionを開く。Agent用connection自身による`ATTACH`は禁止し、実行中に他Plane DBの変化を直接参照させない。
...
状態遷移、test exit code、Egress block/forward結果、Artifact digestなど、Harnessが観測したもの。
...
Wiki Agentは作業Taskを実行せず、外部networkやEgress Grant権限を持たない。
