...
利用者からは一つのローカルCLI applicationとして提供するが、MemoryとGovernanceは独立processにする。Core、Memory、Governanceはそれぞれ所有SQLiteへだけwriteし、Outbox/InboxとUnix domain socketで非同期messageを配送する。Evidenceを個別ファイルへfallbackしない。技術選定と再評価条件は[13-technology-stack.md](13-technology-stack.md)を正本とする。

## 2. 実装順序

### Phase 1: Task Runtime

- Go Coreのservice run groupとgraceful shutdown
- `control.db` migration、Core Inbox/Outbox
- UDS message transportとSchema validation
...
- Governance Inbox/Outbox、ControlとのACK/reconciliation
...
- Memory Inbox/Outbox、Coreとのterminal snapshot transfer
...
- Memory Inboxへの重複配送を`message_id`で一回だけ適用する
...
- Governance Outbox commit後にCLIへblock responseを返し、Control Inbox再送後もTask Mailboxを重複させない
...
- Grant/Authority待ち中のTask cancellationでCore Action gateを先に閉じ、Governance revoke ACK後にAsyncCancelledへ一度だけ収束する
...
- Governance activation TransactionでGrant activeと結果Outboxを確定し、Control Inbox適用TransactionでRequest/Async completedとReady Eventを同時確定する
...
- Authority期限切れでControl Decision、Governance deny、Control Async/Mailbox終端を冪等message chainとして収束させる
...
- Workspace freeze/archive/destroyが先にCore Action gateを閉じ、Governance ACKまでcleanup完了にせず、GrantとReady Eventを無効化する
