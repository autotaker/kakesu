...
Work Agent traceはRoot Taskから子Task、Response、async operation、Effectへ伝播する。組み込みAgentではprovider Response IDやcall IDを永続traceへ含めず、機能固有の`review_id`または`job_id`と入力digestを使う。
...
- Policy Judgeを単一の一時API sessionで実装
