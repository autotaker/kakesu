...
| `reviewer` | `reviewer` | `Terra / medium` | DEVから独立し、差分と`make check`をレビューする。 |
| QA | `qa` | `Terra / medium` | DEVから独立し、マージ後の受入れとFAIL分類を行う。 |
| 限定調査 | `explorer` | `Luna / medium` | 一問、`read-only`、再委譲禁止。 |

mainは`PLAN → DEV → review → QA`を一体ずつ順番に進める。DEVとレビュアー、DEVとQAは兼任させず、ネイティブ`Spawn Agent`への切替を理由に承認、統合、`merge`、FAIL分類を子へ移譲しない。
