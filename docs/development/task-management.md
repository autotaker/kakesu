...
REVIEW修正後にMainが結果を引き継ぐ場合は、[QAガイドライン](qa.md)の閉じた`CF-1`から`CF-7`を`QA_RESULT.md`へ記録する。影響QAケース集合が空でなければ該当ケースを再実行し、影響を限定できなければfull rerunとする。禁止条件が一つでも真または不明なら引き継がない。マージ後は`merge_tree`を承認案 treeと比較し、環境依存ケースだけを限定確認する。
