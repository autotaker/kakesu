...
安全契約変更では、`TASK.md`に製品コード、テスト、ランタイム・ビルド設定、Schema、製品依存、生成製品入力/成果物、外部観測可能な挙動を変更しないと明記する。完了にはPLANの独立計画レビューと分類承認、HANDOVERの対象検査ダイジェストと案/merge tree、実際のno-ff merge差分の許可パス一覧との照合を必要とする。製品用のREVIEW/QA PASS、製品用の完了HANDOVER、Wiki取込記録は要求しない。
