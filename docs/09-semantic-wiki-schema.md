...
意味層はMarkdown本文を正とし、YAML フロントマターは文書種別と表示名だけにする。
...
更新日時はGit履歴、関連はMarkdown リンク、根拠は本文中のエピソード リンクで表す。フロントマターへ`sources`、`status`、`maturity`、`related`を重複保持しない。
...
| `case-pattern` | どんな条件で何が起きやすいか | エピソード群から得た経験的パターン |

## 3. 概念

概念は辞書的一文ではなく、対象を識別・区別するためのまとまりである。
...
Schemaは複数概念を、1つの状況を理解する構造へ配置する。
...
スクリプトは絶対的なワークフロー定義とは限らない。実際のTaskがどのように進むと期待されるかを表す認知モデルである。

実例: [../examples/semantic/scripts/task-completion.md](../examples/semantic/scripts/task-completion.md)

## 6. ケース パターン

ケース パターンは個別エピソードに近いが、複数状況で再利用できる経験的形を持つ。
...
フロントマターの`sources`は持たない。本文の説明単位ごとにエピソードへリンクする。
...
複数文が同じ根拠を共有する場合、段落末尾にまとめる。ページ末尾のSources一覧は補助であり、本文リンクを正とする。
...
Wiki AgentはMarkdown リンク グラフを検索・探索に利用する。
...
一ページに複数種別を混在させない。たとえばTask 所有権ページに完了手順を長く書かず、Task 完了 スクリプトへリンクする。
...
既存構造では説明できないため、概念境界、役割、関係、スクリプト分岐を修正する。

## 11. 成熟度の扱い

`maturity: stable`のような機械的ラベルはフロントマターへ置かない。
...
一時的な議論はエピソードか設計成果物に残し、早期に意味ページへ昇格させない。

## 13. Wiki Agent向け編集規約

- 現在Taskの目的を書き込まない
- 一エピソードのオーナー assertionを一般則にしない
- 反例を削除しない
- 記述の根拠を段落単位でリンクする
- 同じ説明をフロントマターと本文に二重化しない
- 既存リンクを保つかリダイレクトページを作る
- 歴史的経緯は必要な場合だけ短く説明し、詳細はエピソードへリンクする
...
過去ページの文章が現在の命令に見えないよう、記憶 拒否として明示する。
