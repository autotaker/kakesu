...
このディレクトリ以下の仕様・設計を更新するときは、文章だけで完了扱いにしない。変更した責務、状態、メッセージ、永続化境界がSchemaとE2E traceへ影響するかを確認する。

## 必須確認

1. `rg`で同じ概念、状態名、責務、メッセージ type、旧仕様の残存箇所を検索する。
2. Plane責務を変更した場合は、全体設計、該当PlaneのSchema catalog、データモデル、代表E2Eの記述を整合させる。
3. 状態遷移またはPlane間メッセージを変更した場合は、`examples/e2e-tabletop`のactive trace、canonical ペイロード、sequence requirementを更新する。
...
Viewerに関係する変更では、再生成後の`viewer-data.js`をコミット対象に含める。件数だけでなく、変更したscenarioのメッセージ chainとcanonical ペイロードが意図どおり表示されることを確認する。
...
- Schema review: canonical coverage、required フィールド、ID join、状態整合、未使用ペイロード、validatorの偽PASSを確認する。
