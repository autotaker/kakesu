...
永続化前に共通Redaction Pipelineを通す。Credential、Authorization header、Cookie、Secret環境変数、Policy指定PIIは平文保存しない。Egressではredacted metadata、digest、size、classificationを常に残し、事後調査用のbounded sanitized contentまたはencrypted captureを原則全通信についてEvidence DBへ短期保存する。Capture Manifestには保存byte range、redacted range、truncation、欠落理由、completion statusを明示する。high-riskまたはFinding関連だけを長期保持へ昇格し、Credentialはcapture前に除去する。inspection不能な通信はcontentの代わりにflow metadataと観測限界を記録する。
