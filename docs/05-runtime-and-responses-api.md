...
1つのAgentを、Contextを読み、Actionをyieldし、結果イベントで再開するコルーチンとして扱う。
...
モデルが返すFunction CallをHarness Actionとして解釈する。Responseの`call_id`に対応する`function_call_output`を次入力へ渡して推論を続けられる。
...
- `instructions` parameterは、そのResponseのContextへsystemまたはdeveloper メッセージとして挿入される
- `previous_response_id`を指定しても、前Responseで指定した`instructions` parameterは次Responseへ引き継がれない
- `input` itemとして明示した`role: "developer"` メッセージと、top-level `instructions` parameterを混同しない
...
1回の長いモデル推論を非同期実行する補助手段として利用できる。Task scheduler、Mailbox、子Agent管理の代替ではない。
...
同じTaskへ同時に2つのResponse Stepを走らせない。楽観ロック用に`task.version`または`run_step_seq`を使う。

## 7. Agent Run Record Policy

本節を、Agent Runから発生する情報の記録範囲・正本性・保持方針の正本とする。Task Event、Egress Audit、Artifactなど別Aggregateが正本を持つ情報は、Agent Run側に複製せず参照を保存する。
...
`agent_runs`は実行セッション、`agent_run_steps`は1回のResponses API呼び出し、`agent_run_items`は完成した出力 itemの正規化記録を表す。Streaming deltaをEvent Sourcingの単位にはしない。
...
| Step metadata | step sequence、レスポンス ID、種別、開始終了、status | Run履歴の正本 | 長期 |
| Request context | Contract／State／Progress／Mailbox等のバージョンと参照、リクエスト ダイジェスト | 各Storeが正本 | 長期 |
| 完全なリクエスト body | 原則保存しない。必要時だけEvidence DBの暗号化BLOB | 派生コピー | 短期 |
| 完成出力 item | item type、ID、status、必要フィールド、raw ダイジェスト | Run Evidence | Policy依存 |
| 通常メッセージ text | textまたは暗号化blob参照 | Run Evidence | 短期、Artifact昇格可 |
| Function Call | name、call ID、検証済みarguments、schema バージョン | Tool要求の正本 | 長期 |
| Function Call Result | call ID、status、result／エラー参照 | Tool結果Storeが正本 | 長期参照 |
| Hosted Tool item | tool種別、item ID、status、result参照 | Provider出力の記録 | Policy依存 |
| Task状態へ作用した結果 | Task Event ID | Task Eventが正本 | 長期参照 |
| Ask／Escalation／Review | リクエスト／decision ID | 各Aggregateが正本 | 長期参照 |
| Async Operation | async IDとresult ref | Async Storeが正本 | 長期参照 |
| Artifact／Workspace | immutable Artifact ref、スナップショット ref | 各Storeが正本 | 長期参照 |
| Usage | 入力、cached 入力、出力、reasoning等のtoken数 | 課金・観測記録 | 長期集計、明細はPolicy依存 |
| Error | エラー code、再試行able、provider リクエスト ID、redacted detail ref | Run障害記録 | 長期 |
...
- 重複したContext本文。バージョン、参照、ダイジェストから再構築できるものは複製しない
...
standalone `/responses/compact`の出力はAPI仕様に従い全体を変更せず次入力へ渡す必要があるため、Evidence DBへ暗号化BLOBとして保持する。successor Stepが`completed`として永続確定するか、`failed`かつ再構築可能なリクエスト Snapshot／Resume Cursor／非再試行決定がコミットされるまでは削除しない。`consumed_by_step_id`と参照状態を保存し、そのdurable boundary後だけRetention Policyに従って削除できる。
...
- 失敗時は`ProgressRefreshFailed` Eventとredacted エラーを保存する

### RedactionとSecret

永続化前に共通Redaction Pipelineを通す。Credential、Authorization header、Cookie、Secret環境変数、Policy指定PIIは平文保存しない。Egressではredacted metadata、ダイジェスト、size、classificationを常に残し、事後調査用のbounded sanitized contentもしくはencrypted captureを原則全通信についてEvidence DBへ短期保存する。Capture Manifestには保存byte range、redacted range、truncation、欠落理由、completion statusを明示する。high-riskまたはFinding関連だけを長期保持へ昇格し、Credentialはcapture前に除去する。inspection不能な通信はcontentの代わりにflow metadataと観測限界を記録する。

Function argumentsにもSecretが入りうるため、Tool schemaごとのsensitive フィールド指定と汎用Secret scannerを併用する。監査ダイジェストはredaction前ペイロードから安全な境界内で計算し、表示用recordはredactする。

### Transaction境界

1つのStepについて、完成出力 item、Tool dispatch、Task Event参照、Continuation、Step statusを再実行可能に保存する。外部Tool実行をDB Transaction内に抱えず、開始前にdispatch intentをコミットし、結果を別Transactionで確定する。
...
Crash時は`response_id`、`output_item.id`、`call_id`とHarness生成Operation Keyで重複適用を防ぐ。LLMへ`idempotency_key`を生成させない。
...
CompactionによってTask status、Owner Assignment、Contract バージョン、未完了Operationを変更しない。Taskが`running`なら`running`のままであり、`waiting`や`reviewing_completion`でもその状態と再開条件を維持する。
...
- stateless 入力-array chainingではcompaction itemを含む出力を次入力へ追加し、必要なら最新compaction itemより前だけを削除する
- `previous_response_id` chainingでは新しいメッセージだけを渡し、手動pruneしない
- standalone `/responses/compact`を使う場合は、返されたcompacted window全体を変更せず次のResponses 入力へ渡す
...
Harnessは既存の非終端itemが応答から消えていないこと、item IDの一意性、Evidence参照、progress バージョン、Event watermarkを検証する。削除相当のitemは省略せず`cancelled`として残す。

`update_progress`を保存した後、Harnessは同じ`call_id`の`function_call_output`を永続化する。次の通常Responseを同じchainで続ける場合は、その出力を次入力へ渡す。Progress更新後に追加のMaintenance用メッセージ生成を要求しない。
...
HarnessはCursor保存前に、Task・Agent・Contract・Progress バージョンの一致、Workspace参照の存在、EventとMailbox sequenceの単調性を検査する。検査に失敗した場合は旧Runを閉じず、最新状態から再生成する。旧Response chainを利用できずCursorも作れない場合は、Taskを`suspended`としてOperator復旧へ送る。

新Run作成前にContract、Task、Progress バージョンが変わった場合、Cursorをstaleとして破棄し、最新状態から再生成する。
...
3. window外でも未解決のTool Call／Async開始、エラー、Ask／Escalation、Decision、Artifact生成、Progress Refresh

入力token budgetを超える場合は、未観測Event、未解決Event、Decision／エラー、直近Eventの順で優先する。省略したEventは件数とsequence範囲だけを示し、必要ならAgentがArtifactやRun RecordをTool経由で確認できるよう参照を付ける。
...
Reasoning、opaque compaction item、Secret、token単位deltaは注入しない。通常メッセージも全Transcriptとして戻さず、Retention内にある直近textまたはredacted summaryだけを対象とする。Agent Run Eventsは再開補助であり、Current Contract、State、Progressを上書きしない。
...
- `request_grant`はTaskとimmutable Challengeを照合し、同じ`call_id`の再配送を同一Grant Requestへ収束させる
