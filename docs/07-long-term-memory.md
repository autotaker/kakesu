...
HarnessはTask終端EventからJobを冪等に生成する。Job失敗は終端Taskの状態を戻さず、再試行後も解消しなければ`needs_operator`にする。
...
Episode Agentは不足する詳細をRead-only Toolで取得する。

### Read-only Evidence Tools

```text
get_task_contract_versions
get_task_outcome
get_task_progress_history
list_task_events / get_task_events
list_agent_runs / list_agent_run_steps / get_agent_run_items
list_child_task_outcomes
list_ask_and_escalation_decisions
list_completion_reviews
list_effect_records
get_artifact_metadata / read_artifact
```

一覧ToolはID、時刻、種別、短いmetadata、page cursorを返し、詳細Toolは指定範囲だけを返す。Episode Agentは必要なEvidenceを選択して複数Stepで調査できる。
...
各Responses API呼び出しには、最初からRead-only ToolsとTask Episode用`text.format: json_schema`を同時に指定し、`tool_choice: "auto"`とする。

```typescript
responses.create({
  model,
  input,
  tools: episode_read_tools,
  tool_choice: "auto",
  text: {
    format: {
      type: "json_schema",
...
sequenceDiagram
    ...
    H->>E: terminal Task index + outcome
    loop Evidence investigation
      E->>H: read-only Evidence Tool Call
      H-->>E: function_call_output
    end
