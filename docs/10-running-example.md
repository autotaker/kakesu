...
`git push`と`gh pr create`は別の通信系列であり、Grantを共有しない。まずGit HTTPS通信が`challenge-481`としてblockされ、同じ`request_grant → PolicyGrantReady → command再実行`を経てremote branchを作る。以下では続くPR作成側の`challenge-482`を示す。一つのCLI commandが複数のcanonical requestを送る場合も、未許可requestごとに同じworkflowを繰り返す。
...
HarnessはMailboxをAgent Contextへ入れる際、これらのrefからsanitized summaryとして`api.github.com:443`、`POST /graphql`、body digestを併記する。
...
Policy Agentが`grant`を返すと、CASB Policy ManagerがChallengeから同一Workspace・同一request digestに一回だけ使えるexact HTTPS Ruleを生成・反映し、`PolicyGrantReady`をMailboxへ送る。L2は`gh pr create`を再実行する。Credential BrokerがGitHub credentialを外側requestへ注入し、PR #482が作成される。最初にblockしたrequestをGatewayが自動再生することはない。
