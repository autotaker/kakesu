...
Workspace network identityはnetwork namespace、cgroup、AppContainer等のOS adapterから付与し、通信内容をAgentの自己申告に依存してAgentやTaskへ関連付けない。Rule Engineは`workspace_id`からSecurity Policy Bindingを解決し、Task/Agentは監査provenanceとして記録する。Control Planeが利用不能な場合はfail closedとする。
...
Ruleはdestination、protocol、port、Workspace/Profile、Credential scope、method/path、size、data classification、content pattern、期限、使用回数を条件にできる。複数Ruleのpriorityとdeny-overrides規則を固定し、未分類、Rule conflict、Control Plane障害は既定でrejectする。
...
block通知の正本はTask Mailboxである。CLIへ返すHTTP 403、response header、socket errorは即時feedbackだがbest effortとする。

```text
persist EgressAttempt
  → persist EgressChallenge
  → persist Mailbox Outbox(EgressBlocked)
  → commit
  → CLIへblock response
  → Task Mailboxへat-least-once delivery
```
...
Grant作成直前にWorkspace/Policy Binding active、source Task active、Challenge期限、Contract/delegation、binding digest、Policy version互換性、DNS Snapshot freshnessを再検査する。Policy Grantは元のGrant RequestとGrant Decision Recordを必須参照し、Authority経由ならapprove済みAuthority Decisionも必須参照する。最初のTransactionではGrant Decision、`pending_activation` PolicyGrant、Policy version、pending Ready Outboxを確定するが、Grant RequestとAsync Operationは終端しない。Workspace/protocolのauthoritative Enforcement PointがそのPolicy versionをACKした後のactivation Transactionで、Binding active、Request非終端かつGrantが`pending_activation`であることをlock下で再検査し、Grantを`active`、Grant RequestとAsync Operationを`completed`にして`AsyncCompleted`と`PolicyGrantReady`を配送可能にする。EventにはGrant Request ID、Async ID、`policy_version`を含める。`AsyncCompleted(result_ref=GrantResult)`を正本とし、`PolicyGrantReady`はCLI再実行を促す補助Eventとする。最初にrejectした通信をGatewayが自動再生せず、Agentが元のCLI commandを再実行する。
...
回答は認証済みresponder principal、decision、rationale、時刻をimmutableな`GrantAuthorityDecision`へ一件だけ保存する。回答TransactionはRequestをlockして未解決・未期限切れを検証し、approve後もCASB Policy ManagerがWorkspace/Policy Binding、source Task、Challenge、binding、Policy/DNS freshnessを再検査する。期限切れworkerはAuthority Requestを`expired`、Grant Requestを`denied`、Async Operationを`completed`にし、deny結果とMailbox Outboxを原子的に確定する。期限後の回答や重複回答は状態を変えず、既存の終端結果を返す。
...
Task cancellation時はsource Task、Workspace、Grant Request、ChallengeをGrant適用側と同じlock順で取得し、そのTaskが申請した未解決Request/Authority Request/Evaluation Jobをcancelし、source Taskに紐づく`pending_activation`または`active` Grantをrevokeし、pending Ready Outboxを無効化し、`AsyncCancelled`を同一Transactionで作る。WorkspaceのBaseline Policy Binding自体は削除しない。新規connectionを直ちに拒否し、P0では既存connectionも短いgrace period後に終了する。

Workspaceを`frozen | archived | destroyed`へ遷移する場合はPolicy Binding、Grant Request、Authority Request、Evaluation Job、Async Operation、pending/active GrantをGrant側と同じlock順で取得する。同一Transactionで未解決workflowをcancelし、Grantをrevokeし、pending Ready Outboxを無効化して`AsyncCancelled`を確定してから新規Egressを停止する。Grant作成・activation・Authority回答側もWorkspace/Binding activeをlock下で再検査する。Owner Agentの交代、Agent Run停止、Context CompactionだけではPolicy BindingやGrantを変更しない。

Agentの`cancel_async`はGrant activation前だけ取消可能で、Grant Request、Authority Request、Evaluation Job、Async Operationをcancelし、既存の`pending_activation` Grantをrevokeし、pending Ready Outboxを無効化して`AsyncCancelled`を原子的に確定する。activation後は`not_cancellable`を返し、暗黙にGrantをrevokeしない。明示的なGrant revokeは将来の別操作とする。既に外部で成立した作用はrollbackせず、必要ならAgentが通常のCLIで補償操作を行い、その通信も同じCASB Policyを通る。
...
1. Attempt、Rule Decision、Outbound Transaction、Workspace、Task、EvidenceをIncidentへ固定する
2. critical時はOperatorへ通知し、関連Task/Grant/connectionの停止またはrevokeを別の決定として行う
3. bypassの再現caseをEval datasetと回帰テストへ追加する
4. Policy Agentがcandidate RuleとEvalを作り、対象Workspaceまたはglobal baselineを明示したversioned Policy Revisionとして修正する
