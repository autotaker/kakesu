...
Taskの状態遷移は[02-task-lifecycle.md](02-task-lifecycle.md)、一回ごとの推論実行とResponses APIの対応は[05-runtime-and-responses-api.md](05-runtime-and-responses-api.md)を正本とする。
...
Agent Runから発生するrequest、response item、Tool Call、usage、errorの保存範囲は[05-runtime-and-responses-api.md](05-runtime-and-responses-api.md)の「Agent Run Record Policy」を正本とする。
...
論理Workspaceのsnapshot、Artifact、Evidence固定はTask Outcomeの責務であり、固定内容はEvidence DBへ取り込む。一方、Git worktreeやcontainer volumeなどの物理実体はAgent Resourceであり、DB取込とOutcome参照の固定後に削除できる。
