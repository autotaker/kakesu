...
Policy Judge AgentはExternal Effect要求をPolicy Cascadeと照合する統治Planeの組み込みAgentである。Work Agentや親Ownerから独立し、`allow | deny | require_authority | insufficient_information`と根拠をStructured Outputで返す。Credentialを取得せず、Effectを実行しない。Governanceコンポーネントが検証済み判定をEffect Gatewayへ渡す。詳細は[07-governance.md](07-governance.md)を正本とする。

Responses APIには[schemas/built-in-agent-outputs.json](../schemas/built-in-agent-outputs.json)の`policy_decision`を`text.format`として指定し、判定確定用Function Toolは渡さない。
