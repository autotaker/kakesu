...
validなManifestは承認候補となるcontentを一意に表すだけであり、**approved、granted、pushableを意味しない**。Approval state storeが現在時刻、TTL、request ID一意性、policyの信頼性を所有し、Passkey challengeとone-shot push grantはその後続の別境界である。Manifest自身はGit receive-pack/pkt-lineを解析せず、remoteのold SHAを観測せず、forceをwireから推定せず、credential、実push、audit、network又は永続stateを持たない。
