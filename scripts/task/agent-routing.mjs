...
export const ROLE_CONTRACTS = Object.freeze({
  ...
  wiki: { profile: "terra-medium", model: "gpt-5.6-terra", effort: "medium", sandbox: "workspace-write" },
});
...
const PROJECT_ROLE_REGISTRY = Object.freeze({
  ...
  wiki: { description: "Wiki editing only using Terra/medium.", config_file: "agents/wiki.toml" },
});
...
export function readCanonicalContracts(productRoot = MODULE_ROOT) {
  ...
  for (const [role, expected] of Object.entries(ROLE_CONTRACTS)) {
    ...
    if (role === "explorer") {
      ...
      if (agents === null || !/^max_threads\s*=\s*1\s*$/m.test(agents) || !/^max_depth\s*=\s*0\s*$/m.test(agents)) {
        throw new Error(`ROUTING_WIKI_CHILD_POLICY_MISMATCH: ${role}`);
      }
      if (!/editing only/i.test(content) || !/Do not .*spawn/i.test(content) || !/stage.*commit.*merge/i.test(content) || !/\.git/i.test(content)) {
        throw new Error("ROUTING_WIKI_EDIT_ONLY_POLICY_MISSING");
      }
    } else {
      const agents = tomlSection(content, "agents");
...
function redactDiagnostic(value, limit = 240) {
  ...
  const result = value
    ...
    .replace(/[\r\n\t ]+/g, " ")
    .trim();
  return (result || "(no diagnostic)").slice(0, limit);
}

export function buildLaunchEvidence({ route, cwd, allowedPaths, childResult = null, commit = null, error = null }) {
  return {
    event: "agent_launch",
    route: "fixed-role",
