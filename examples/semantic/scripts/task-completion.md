...
2. HarnessがOwner、Contract version、required child、Artifact参照を検査する。
3. HarnessがOutcomeとEvidenceのdigestを固定する。
...
`reviewing_completion → waiting_evidence`。Ownerが追加Evidenceを提出して再Reviewする。

## 典型的な失敗

- Ownerの説明だけでOutcomeがない。
- 古いContract versionで完了候補を出す。
