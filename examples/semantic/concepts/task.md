...
- Ownerは必ず一人である。
- 1つのOwnerは同時に1つの非終端Taskだけを処理する。`waiting`中もOwnerは占有される。([Owner排他の設計Episode](episode://design/task-owner-exclusivity))
...
shell command、ファイル読取、1回のTool Call、同じOwnerによる一時的な仮説検証は通常Activityである。長時間かどうかではなく、独立Ownerへ責任を移したかで区別する。([Task境界の設計Episode](episode://design/task-boundary))
