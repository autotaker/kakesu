...
| QA-4: normal comprehensive result | evidence-review | Audit the DEV-supplied, candidate-bound root `make check` exit status and complete log. Verify it ran after the candidate was fixed, passed, and supports the expected command ordering without duplicate `lint-docs` execution. QA does not rerun it, preventing duplicate comprehensive build/test work. Independently run `git diff --check` against the candidate worktree (or audit an equivalent candidate-bound result) and require PASS. |
