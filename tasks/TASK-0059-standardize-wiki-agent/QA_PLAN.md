...
期待値の正本は `TASK.md` の `Planning input packet` だけとする。PLAN、実装案、DEV自己申告は期待値の根拠にしない。これはproduct変更として、candidateの9 product workflow pathと、completion transactionでのみ統合するMain管理の`wiki/AGENTS.md`差分を分離して評価する。Kakesu/runtime、Schema、依存、生成入力/成果物、外部観測可能な製品挙動は変更しない。
...
| QA-005 | AC-5, AC-6 | launcher固有fixtureが削除され、残るfocused process testsがQA-001〜004の不在、標準role、Main所有、receipt任意性を失敗検出することを確認する。candidate diffは`wiki/AGENTS.md`を除く9 product workflow pathに限定され、`scripts/task/unified-lifecycle.mjs`、`scripts/task/check-task.mjs`および安全契約candidate例外を含まないことを確認する。DEVのroot `make check`を含む同一candidate証跡をcommand/cwd/resultまで独立監査し、QAはroot full checkを重複実行しない。 | `evidence-review` / candidate diff、focused test本文とcandidate-bound DEV証跡でproduct workflow差分を監査できる。 |
...
zero exitだけでは不十分であり、対象testの欠落、skip、弱体化、candidate不一致、またはnegative scenarioの欠落は該当ケースをPASSにしない。`make wiki-index`、`make work-check`、docs lint、root `make check`、`git diff --check`はQA-005のDEV evidence review対象であり、このfocused rerunに加えてQAが再実行しない。安全契約Taskだけのcandidate例外、`scripts/task/unified-lifecycle.mjs`、`scripts/task/check-task.mjs`のQAは本計画から撤回する。
...
- [ ] candidateの9 workflow path、merge前main-side `wiki/AGENTS.md`差分、completion後のKakesu/runtime非変更とreceipt optionalを確認し、製品QA_RESULTを作成した。live E2Eは計画していない。
