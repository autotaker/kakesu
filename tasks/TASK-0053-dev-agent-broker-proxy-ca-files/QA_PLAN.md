...
`tools/dev-agent-harness/internal/brokercredentials/` と `tools/dev-agent-harness/README.md` だけを独立確認し、`proxyca` の意味、listener/session/broker composition、設定/CLI/service binary、生成物、依存を変更していないことを確認する。
...
- [ ] fixed basenames、descriptor/no-follow、metadata/hardlink/race、all-or-nothing、proxyca delegation/dynamic clock、non-leak API、public-copy/two-host、credential/JWT/parallel isolation の failure detection を確認する。
- [ ] 変更が許可 path と追加＋削除1,000行以内に収まり、既存 dependency/package の意味を変えていないことを確認する。
- [ ] provision/generate/rotate/trust/client/VPS live-e2e を PASS に置換せず、期待値又は scope を変更していないことを確認する。
